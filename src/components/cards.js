...
// Card component receives 'filterr', 'obj', and 'user' as props
const Card = ({ filterr, obj, user }) => {
  ...
  return (
    <div className='card-container box-shadow'>
      <div className='card-head'>
        {/* Displaying the task ID and user profile image */}
        <span>{obj.id}</span>
        <ProfileImage name={user[obj.userId].name} available={user[obj.userId].available} />
      </div>
      {/* Displaying task status icon (if not grouped by status) and title */}
      <h3>{filterr?.grouping !== "status" && status_map[obj.status]} {obj.title}</h3>
      <div style={{ display: 'flex', alignItems: 'center' }}>
        {/* Displaying priority icon (if not grouped by priority) */}
        {filterr?.grouping !== "priority" && priority_map[obj.priority]}
        {/* Displaying tags */}
        {obj.tag.map((ele, i) => (
          <p key={i}>
            <i style={{ color: '#bec2c8', marginRight: '4px' }} className='bx bxs-circle'></i>{ele}
          </p>
        ))}
      </div>
    </div>
